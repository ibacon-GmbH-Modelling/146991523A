%% BYOM, simple DEBtox model: byom_debtox_daphnia_batch_epx.m
%
% *Table of contents*
...
% making batch-window predictions for FOCUS profiles.
% 
...
% % Note that first input can be par, but if left empty it is read from the
% % saved file. Second input is a vector with time points on which ECx is
...
% done, with a fixed window of 21 days, and several fixed MFs. This will be
% a relatively fast and practical way to see if a certain critical safety
% margin is available. Next, the LPx can be calculated exactly.
% 
...
opt_ecx.mf_range  = [3 10 30 100 300 1000]; % range for fixed MFs to make plots with for calc_effect_window
opt_ecx.mf_crit   = 30; % MF trigger for flagging potentially critical profiles (EP10<mf_crit)

[file_out,Tflagged] = calc_effect_window_batch(par,Twin,opt_ecx,opt_conf);
% Note: the batch calculations will always be performed without CI at the moment.
% 
% Note: this function outputs the file names for the profiles that are
% flagged as potentially interesting (by default EP10<30), as well as the
% start time of the window where this occurs. This is NOT necessarily the
% same window for the lowest EP10!
% 
% Note: when using the parallel toolbox, note that profiles are run
% sequentially but MFs from opt_ecx.mf_range in parallel. It is most
% efficient to optimise the number of elements of this range in relation to
% the number of workers used for parallel processing. With a PC of up to 6
% physical cores, there will automatically be n_cores-1 workers. With more
% cores available n_cores-2. On an 10-core machine, you can therefore use a
% range of 8 elements efficiently. On a 4-core machine, it is better to use
% 6 (or 9, in any case: multiples of 3).

return

% We can continu calculating explicit EPx values for the flagged profiles.
% Here, this is illustrated using the *first* flagged profile only.
% Consider doing this in a for loop.
fname_prof = file_out{1}; % take first of the flagged profiles
...
% the window of choice, or use Tflagged(i) if you want to trust the
% effect-window calculation.
