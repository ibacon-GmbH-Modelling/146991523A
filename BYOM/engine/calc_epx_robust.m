...
if batch_epx == 0
     f = waitbar(0,'Calculating robust EPx. Please wait.','Name','calc_epx_robust.m');
end
...
for i = 1:length(MF_test)
    ...
    if batch_epx == 0
        waitbar(i/length(MF_test),f) % make a nice waiting bar
    end
    
    glo.MF = MF_test(i); % modify global
    Xout   = call_deri(t,par_plot,X0mat_tmp,glo); % use call_deri.m to provide the output for one scenario
...
if type_conf > 0 % if we make CIs ...
    ...
    if batch_epx == 0
        f = waitbar(0,'Calculating confidence intervals on robust EPx. Please wait.','Name','calc_epx_robust.m');
    end
