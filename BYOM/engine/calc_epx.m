...
if type_conf > 0 % if we make CIs ...
    ...
    for k = 1:n_sets % run through all sets in the sample
        ...
        % the sample.
        Xout   = call_deri(t,par_k,[0;X0mat_tmp(2:end)],glo); % use call_deri.m to provide the output for one scenario
        Xctrl  = Xout(end,ind_traits); % remember the relevant output for the traits
        ...
        for i_X = 1:length(ind_traits) % run through traits
            for i_F = 1:length(Feff) % run through effect levels
                if ~isnan(EPx{i_F}(i_X))
                    % use fzero to zero in on the exact value
                    EPx_tmp = fzero(@calc_epx_sub,EPx{i_F}(i_X),[],t,par_k,Feff(i_F),X0mat_tmp,Xctrl(i_X),ind_traits(i_X),glo); % find the EPx,t
...
% Usage: crit = calc_epx_sub(MF,t,par_out,Feff,X0_mat,Xctrl,locX,glo)
%
...
glo.MF = MF; % modify glo's multiplication factor
