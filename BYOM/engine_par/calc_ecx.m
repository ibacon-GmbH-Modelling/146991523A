    ...
    for i_X = 1:length(ind_traits) % run through traits
        ...
        for i_F = 1:length(Feff) % run through effect levels
            ...
            if numel(EC_range) == 2
                ...
                ECx{i_F}(i_T,i_X) = fzero(@calc_ecx_sub,EC_range,[],Tend(i_T),par_plot,Feff(i_F),X0mat_tmp(2:end),Xctrl_tmp(i_X),ind_traits(i_X),glo); % find the ECx,t
            else
        ...
        for i_T = 1:L_i_T % run through time points
            ...
            for i_X = 1:L_i_X % run through traits
                ...
                for i_F = 1:L_i_F % run through effect levels
                    ...
                    a = calc_ecx_sub(Cminmax(1),Tend(i_T),par_k,Feff(i_F),X0mat_tmp(2:end),Xctrl_tmp,ind_traits(i_X),glo_tmp); 
                    b = calc_ecx_sub(Cminmax(2),Tend(i_T),par_k,Feff(i_F),X0mat_tmp(2:end),Xctrl_tmp,ind_traits(i_X),glo_tmp);
                    ...
                    if ~isnan(ECx{i_F}(i_T,i_X))
                        ...
                        elseif b >= 0
                            ...
                            ECx_tmp = fzero(@calc_ecx_sub,ECx{i_F}(i_T,i_X),[],Tend(i_T),par_k,Feff(i_F),X0mat_tmp(2:end),Xctrl_tmp,ind_traits(i_X),glo_tmp); % find the ECx,t
                        end
...
function crit = calc_ecx_sub(c,Tend,par_out,Feff,X0,Xctrl,locX,glo)

% Usage: crit = calc_ecx_sub(c,Tend,par_out,Feff,X0,Xctrl,locX,glo)
%
...
Xtst2 = call_deri([0 Tend/2 Tend],par_out,[c;X0],glo); % response at conc. c
