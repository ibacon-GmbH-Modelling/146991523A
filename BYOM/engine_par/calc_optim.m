...
switch opttype
    ...
    case 5 % Alternative Nelder-Mead simplex
        ...
        [phat,FVAL,icount,numres,ifault] = nelmin(@transfer,length(pfit),pfit,1e-8,0.05*pfit,1,30*length(pfit),pmat,WRAP);
        out_iter = icount; % remember iterations
        % disp(['rough nelmin iteration:',num2str(icount)])
        
        for no_opt = 1:simno-1        
            [phat,FVAL,icount,numres,ifault] = nelmin(@transfer,length(pfit),pfit,1e-15,0.05*pfit,1,200*length(pfit),pmat,WRAP);
        end
...
% disp(' ')
disp('=================================================================================')
disp('Results of the parameter estimation with BYOM (par-engine) version 6.0 beta 4')
disp('=================================================================================')
